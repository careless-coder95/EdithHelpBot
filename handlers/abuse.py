    ...
    async def noabuse_toggle(client, message: Message):
        ...
        if status:
            await message.reply_text(
                "✅ **Abuse Detection ON!**\n\n"
                "Ab gaaliyan dene wale ka message delete ho jaayega. 🚫🤬"
            )
        else:
            await message.reply_text(
                "⚠️ **Abuse Detection OFF!**\n\n"
                "Abuse filter band kar diya gaya."
            )
        ...
        if message.text and ABUSE_PATTERN.search(message.text):
            try:
                ...
                warn_msg = await client.send_message(
                    message.chat.id,
                    f"⚠️ {message.from_user.mention}, gaaliyan dena allowed nahi hai! Message delete kar diya gaya. 🚫"
                )
